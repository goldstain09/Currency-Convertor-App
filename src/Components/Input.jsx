...
export default function Input() {
  ...
  useEffect(() => {
    async function cur() {
      let currency1_data = await fetch(
        `https://v6.exchangerate-api.com/v6/8fd95064ff7ac1a2d34d064f/latest/${currency1}`
      );
  ...
  const InputChange_1 = (v) => {
    ...
    arr.length > 0 &&
      arr.map(([currency, value]) => {
        if (currency === selectedValue2) {
          setInput2(
            `->    ${parseFloat(v * value).toFixed(2)}    ${selectedValue2}`
          );
        }
      });
  };
  ...
  if (loading) {
    return (
      <div align="center" style={{ padding: "10% 40%", width: "100%" }}>
        <CirclesWithBar
          align="center"
          height="300"
          width="300"
      ...
      <div className="container">
        <div className="row justify-content-center">
          ...
          <div className="col col-6 text-dark pt-sm-5">
            <div className="justify-content-evenly d-flex pt-sm-5">
              ...
              >
                ...
                {allCurrency_Data.length > 0 ? (
                  allCurrency_Data.map(([currency]) => (
          ...
          <div className="col col-6  text-dark pt-sm-5">
            ...
            <div>
              {selectedValue2 && <input readOnly type="text" value={input2} />}
            </div>
          </div>
          {/* -------------------------------------------------------- */}
        </div>
